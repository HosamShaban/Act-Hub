import 'package:acthub/core/util/size_util.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';

class ManagerHeight{
  static double h2 = SizeUtil.setHeight(2);
  static double h4 = SizeUtil.setHeight(4);
  static double h6 = SizeUtil.setHeight(6);
  static double h8 = SizeUtil.setHeight(8);
  static double h10 = SizeUtil.setHeight(10);
  static double h12 = SizeUtil.setHeight(12);

}

class ManagerWidth{
  static double h2 = SizeUtil.setWidth(2);
  static double h4 = SizeUtil.setWidth(4);
  static double h6 = SizeUtil.setWidth(6);
  static double h8 = SizeUtil.setWidth(8);
  static double h10 = SizeUtil.setWidth(10);
  static double h12 = SizeUtil.setWidth(12);

}

class ManagerRadius{
  static double h2 = SizeUtil.setRadius(2);
  static double h4 = SizeUtil.setRadius(4);
  static double h6 = SizeUtil.setRadius(6);
  static double h8 = SizeUtil.setRadius(8);
  static double h10 = SizeUtil.setRadius(10);
  static double h12 = SizeUtil.setRadius(12);
}


class ManagerIconSize{
  static double s2 = 2;
  static double s4 = 4;
  static double s6 = 6;
  static double s8 = 8;
  static double s10 = 10;
  static double s12 = 12;


}

class ManagerOpacity{
  static double op0_2 = 0.2;
  static double op0_4 = 0.4;
  static double op0_6 = 0.6;

}